...
# Изменение StepCloud 07/09/2103

class Captcha_Library{
        static private $keystring = "";

        // generates keystring and image
	static function KCAPTCHA(){

		require(dirname(__FILE__).'/kcaptcha/kcaptcha_config.php');
		$fonts=array();
		$fontsdir_absolute=dirname(__FILE__).'/kcaptcha/'.$fontsdir;
		...
		do{
			...
			while(true){
				self::$keystring='';
				for($i=0;$i<$length;$i++){
					self::$keystring.=$allowed_symbols{mt_rand(0,strlen($allowed_symbols)-1)};
				}
				if(!preg_match('/cp|cb|ck|c6|c9|rn|rm|mm|co|do|cl|db|qp|qb|dp|ww/', self::$keystring)) break;
			}
			...
			if($odd==0) $odd=-1;
			for($i=0;$i<$length;$i++){
				$m=$font_metrics[self::$keystring{$i}];
